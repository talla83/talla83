...
#        print "Key %s (%d) was pressed" % (keyname, data.keyval)
    ...
    def on_down_released(self, gtkbutton, data = None):
       if self.hal_led_mts.hal_pin.value == 0:
            self.add_history("Messtaster nicht bereit!")
       else:
        self.command.mode( linuxcnc.MODE_MDI )
        self.command.wait_complete()
        # Start down.ngc
...
#        to_new_xc=self.stat.position[0]-self.stat.g5x_offset[0] - self.stat.g92_offset[0] - self.stat.tool_offset[0] - xcres
    ...
    def on_mta_released(self, gtkbutton, data = None):
        if self.hal_led_mts.hal_pin.value == 1:
            self.add_history("Messtaster bereits aktiv!")
        else:
         self.command.mode( linuxcnc.MODE_MDI )
         self.command.wait_complete()
         # Start mtstart.ngc
         if self.ocode ("O<mtstart> call") == -1:
            return 
    ...
    def __init__(self, halcomp,builder,useropts):
        ...
        self.mta = self.builder.get_object("mta")
        ...
        self.hal_led_mts = self.builder.get_object("hal_mts")
        self.hal_led_mtb = self.builder.get_object("hal_mtb")
        self.hal_led_mtk = self.builder.get_object("hal_mtk")
        ...
        if self.hal_led_mts.hal_pin.value == 0:
            ...
            self.add_history("Messtaster bereit!")
            if self.hal_led_mtb.hal_pin.value == 0:
               self.add_history("Batteriealarm")
